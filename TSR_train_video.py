...
def main_worker(rank, opts):
    ...
    if not opts.MaP:
        ...
        train_dataset = ContinuousEdgeLineDatasetMask_video(sample=opts.ref_frame_num, size=(432,240), split='train', name=opts.dataset_name, root=opts.dataset_root)
        test_dataset = ContinuousEdgeLineDatasetMask_video(sample=opts.ref_frame_num, size=(432,240), split='valid', name=opts.dataset_name, root=opts.dataset_root)
