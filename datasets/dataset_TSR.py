...
class ContinuousEdgeLineDatasetMask_video(Dataset):  # mostly refer to FuseFormer
    def __init__(self, opts, sample=5, size=(432,240), split='train', name='YouTubeVOS', root='./datasets'):
        ...
        # else:
        #     vid_lst_prefix = os.path.join(root, name, 'JPEGImages')
        #     edge_lst_prefix = os.path.join(root, name, 'edges')
        #     line_lst_prefix = os.path.join(root, name, 'wireframes')
        #     self.video_names = [vid_lst_prefix]
        #     self.edge_names = [edge_lst_prefix]
        #     self.line_names = [line_lst_prefix]
        #     mask_lst_prefix = os.path.join(root, name, 'masks')
        #     self.mask_names = [mask_lst_prefix]
