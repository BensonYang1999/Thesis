...
from src_MST.layers import *
from src_MST.lsm_hawp.detector import WireframeDetector
from core.utils import create_dir, stitch_images, to_device, to_tensor
from core.utils import torch_show_all_params
...
class MST:
    ...
    def inference_line_edge(self, items, mask, valid_th, mask_th, output_path=None, not_obj_remove=False):
        
        mask = transforms.functional.resize(mask, (self.img_size, self.img_size), interpolation=InterpolationMode.NEAREST)
        # mask = cv2.resize(mask, (self.img_size, self.img_size), interpolation=cv2.INTER_NEAREST)
        # mask = mask / 255
        # items['masks'] = torchvision.transforms.functional.to_tensor(mask.astype(np.float32))
        items['masks'] = mask.type(torch.float32)
        for k in items:
            if type(items[k]) == torch.Tensor:
                items[k] = items[k].unsqueeze(0)
        items = self.cuda(items)
        ...
        line_edge_tensor = self.postprocess_arbitarySize(lines_edges_pred, items['origin_img'].size()[-2:])
        line_edge_tensor = line_edge_tensor.permute(0, 3, 1, 2)
        line_edge_tensor = torchvision.transforms.functional.rgb_to_grayscale(line_edge_tensor)
        line_edge_array = np.array((line_edge_tensor.cpu())).astype(np.uint8).squeeze()
        line_edge_img = Image.fromarray(line_edge_array)
        
        img_tensor = self.postprocess_arbitarySize(items['img'], items['origin_img'].size()[-2:])
        img_array = np.array((img_tensor.cpu())).astype(np.uint8).squeeze()
        img_img = Image.fromarray(img_array)
        
        mask_tensor = self.postprocess_arbitarySize(items['masks'], items['origin_img'].size()[-2:])
        mask_array = np.array((mask_tensor.cpu())).astype(np.uint8).squeeze()
        mask_img = Image.fromarray(mask_array)
        
        create_dir(output_path)
        print('\nsaving sample ' + output_path)
        inputs = (items['img'] * (1 - items['masks'])) + items['masks']
        inputs_tensor = self.postprocess_arbitarySize(inputs, items['origin_img'].size()[-2:])
        inputs_array = np.array((inputs_tensor.cpu())).astype(np.uint8).squeeze()
        inputs_img = Image.fromarray(inputs_array)
        inputs_img.save(os.path.join(output_path, "inputs.jpg"))
        img_img.save(os.path.join(output_path, "frame.jpg"))
        mask_img.save(os.path.join(output_path, "mask.jpg"))
        line_edge_img.save(os.path.join(output_path, "result.jpg"))

        return line_edge_tensor
    ...
    def postprocess_arbitarySize(self, img, origin_size):
        # [-1, 1] => [0, 255]
        if img.size()[2] != origin_size[0] or img.size()[3] != origin_size[1]:
            img = F.interpolate(img, origin_size)
        img = img * 255.0
        img = img.permute(0, 2, 3, 1)
        return img.int()
