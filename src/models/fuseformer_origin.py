...
from src.models.fuseformer_spectral_norm import spectral_norm as _spectral_norm
...
class Encoder(nn.Module):
    def __init__(self, input_ch=6):
        ...
        self.layers = nn.ModuleList([
            # nn.Conv2d(4, 64, kernel_size=3, stride=2, padding=1),  # add line_edge channel (gray)
            # nn.Conv2d(3, 64, kernel_size=3, stride=2, padding=1),  # original
            nn.Conv2d(input_ch, 64, kernel_size=3, stride=2, padding=1),
...
class InpaintGenerator(BaseNetwork):
    def __init__(self, init_weights=True, input_ch=6, output_ch=2, ref_frames=5):
        ...
        self.encoder = Encoder(input_ch=input_ch)
        ...
        self.decoder = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(64, output_ch, kernel_size=3, stride=1, padding=1)  # for ZITS, output line and edge
        )
    ...
    def forward(self, masked_frames):
        ...
        print(f"fuseformer input : {masked_frames.shape}") # test
